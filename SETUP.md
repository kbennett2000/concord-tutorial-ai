...
| The page says **"Ollama isn't running"** — and `http://localhost:11434` won't load either           | Ollama, the program, isn't started                                                                                   | Start Ollama: on Windows and Mac, launch the Ollama app; on Linux it runs as a background service — `sudo systemctl start ollama`. Then reload the page |
...
| The page's "Thinking…" note sits there a long time                                                  | Nothing is broken — without a graphics card the model works slowly, and the reply arrives all at once when it's done | Give it a moment; if it's regularly painful, switch to the smaller model in [Will it run on my computer?](#will-it-run-on-my-computer)                  |
